# Template: Planning & Resources (evening slot)

**Category:** Planning & Resources
**Reader in mind:** Someone at the kitchen table with a notepad, trying to turn worry into a plan. Often worried about money and paperwork, sometimes on a deadline after a hospital stay. They want steps, checklists, and trustworthy sources.

**Topics this covers:** touring a community, paying for care, long-term care insurance, Medicare and Medi-Cal/Medicaid basics, VA benefits, powers of attorney and advance directives, preparing for a move, hospital discharge planning.

## Voice

- Steady and practical. "Here is what to gather, who to call, and what to ask."
- Honest about complexity. Rules differ by state and by person, so point to official sources and qualified professionals (elder law attorney, financial planner, benefits counselor, the plan's administrator).
- Reassuring without false comfort: "It is a lot, and you do not have to figure it out all at once."
- Use "your parent," "your mom," "your spouse," "a family member." Never "loved one."

## Structure

1. **Intro (2 to 3 short paragraphs).** Acknowledge the weight of the practical side (money, forms, timing). Primary keyword in the first paragraph. Promise a clear, calm path through it.
2. **Section: The basics in plain language.** What the program, document, or process is and is not. Link to an approved official source (for example medicare.gov, medicaid.gov, dhcs.ca.gov, va.gov, nia.nih.gov) where it genuinely helps.
3. **Section: A step-by-step checklist.** Numbered or bulleted steps, documents to gather, or questions to ask.
4. **Section: Common misunderstandings.** Gently correct frequent myths (for example, that Medicare pays for long-term assisted living). Keep claims general and accurate; when unsure, say "check with" the right source.
5. **Section: Who can help.** Professionals and free resources (Area Agency on Aging via eldercare.acl.gov, HICAP counseling in California, elder law attorneys). Link naturally to the services page where relevant.
6. **(Optional) Section:** What to expect at a tour or during move-in planning.
7. **Closing section.** Summarize the first one or two steps, and invite the reader to tour or call with questions. Link naturally to the contact page.

## Sample opening lines (style reference only, do not copy)

- "Paying for assisted living is one of the first questions families ask us, and one of the hardest to get a straight answer to."
- "A good tour is less about the lobby and more about the small moments: how staff speak to residents, and how the dining room sounds at lunch."

## Avoid

- Specific legal, tax, or eligibility advice for the reader's situation.
- Dollar figures, income limits, or coverage percentages without a link to an approved source (and prefer describing how to look them up, since they change yearly).
- Urgency pressure ("act now").
